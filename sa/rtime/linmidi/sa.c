...
memset(&(NT(0)), 0, sine_ENDTBL*sizeof(struct tableinfo));
   ...
   NV(sine_vel) = 
   NS(iline->p[sine_vel]);
NV(sine_a) =  2.0F  * ((float)sin( 7.123788e-05F  * (globaltune*(float)(pow(2.0F, 8.333334e-02F*(-69.0F + NV(sine_pitch) -  12.0F )))))
);
 NV(sine_vel) =  0.5F  * NV(sine_vel) *  7.812500e-03F ;
 NV(sine__tvr0) = NV(sine_vel) *  2.267574e-05F  *  1.050000e+03F ;
 NV(sine__tvr1) = NV(sine_vel) *  9.523810e-04F  *  1.050000e+03F ;
...
NV(sine_incr) =  0.0F ;
 ...
 { NV(sine_incr) = NV(sine__tvr0);
 NV(sine_tot) = NV(sine_tot) + NV(sine__tvr1);
 }
 if  ( NV(sine_rel))
