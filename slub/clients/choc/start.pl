...

{
	...
	my $reserve_strength = 0;
	foreach my $eep (@sorted_eeps) {
	    next if $eep->{strength} < $reserve_strength;
	    next if $eep->{strength} < $racist_strength;
	    
	    my $last_tick = $spreads{$eep->{number}};
	    my $distance = $tick - $last_tick;
	    if ($distance >= $eep->{spread}) {
		...
		my $remaining = $eep->{spread} - $distance;
		if ($remaining < $eep->{racism}) {
		    my $my_reserve_strength = $eep->{strength};
		    $reserve_strength = $my_reserve_strength
		      if $reserve_strength < $my_reserve_strength;
		}
	    }
...
sub on_clock {
	...
	if ($edit) {
	    ...
	    if (not @edits) {
		my $number = $ch - 264;
		my $sample = $samples[($number - 1) % scalar(@samples)];
		my %edit = (chr => $chrs[$number % scalar(@chrs)],
			    number => $number,
			    sample => $sample
			   );
		push(@edits, ({name => "$sample strength",
...
sub draw_samples {
    foreach my $sampleno (1 .. scalar(@samples)) {
	addstr(0, $marginx + (($sampleno - 1) * 9), $samples[$sampleno - 1]);
    }
}
...
sub add_eep {
    ...
    $eeps[$number] = $eep;
    addstr(1, $marginx + (($number - 1) * 9), $eep->{sample});
    addstr(1, $marginx + (($number - 1) * 9), "eep$number$eep->{chr}");
    addstr(2, $marginx + (($number - 1) * 9), $eep->{strength});
    addstr(3, $marginx + (($number - 1) * 9), $eep->{spread});
    addstr(4, $marginx + (($number - 1) * 9), $eep->{racism});
}
