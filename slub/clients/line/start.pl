...
my @watches = ('synth', 'cutoff', 'vol', 'res', 'pan', 
	       'chorus_size', 'chorus_depth', 'rhythm', 'seq'
	      );
...
sub on_clock {
    ...
    # now send change messages
    # this is a bit shit.  we should just flag changes when they're changed
    foreach my $line (@lines) {
	foreach my $watch (@watches) {
	    my $value = $line->{$watch};
	    if (exists $line->{"last_$watch"}) {
		my $last = $line->{"last_$watch"};
		if (ref($value) eq 'ARRAY') {
		    if (scalar(@$value) == scalar(@$last)) {
			for(my $c = 0; $c < scalar(@$value); ++$c) {
			    if ($value->[$c] ne $last->[$c]) {
				send_watch($line, $watch);
				last;
			    }
			}
		    }
...
sub send_watch {
    ...
    my $value = $line->{$watch};
    if(ref($line->{$watch}) eq 'ARRAY') {
	$client->ctrl_send('recap', "$line->{name}_$watch", @{$line->{$watch}});
	$line->{"last_$watch"} = [@{$line->{$watch}}];
    }
    else {
	$client->ctrl_send('recap', "$line->{name}_$watch", $line->{$watch});
	$line->{"last_$watch"} = $line->{$watch};
